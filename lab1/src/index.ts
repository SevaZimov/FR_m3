...
function loadData(url: string, callback: (data: CarData) => void, errorCallback: (error: Error) => void) {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            const data = xhr.response as CarData;
            callback(data);
        } else {
...
function displayData(data: CarData) {
